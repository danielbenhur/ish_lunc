...
def deman_irri(df, parametros=[], pesos=[]):
...
# =SE($H2<=10;$AP2*$AN2;0)
def deman_agri_scbc(df, parametros=['situacao_setor', 'densidade_irri', 'area_setor'], pesos=[1, 1, 1]):
    # H2 - situacao_setor
    # AP2 - densidade
    # AN2 - area_setor
...
# =SOMASE($B:$B;$B2;AQ:AQ)
def deman_agri_otto(df, parametros=['COBACIA', 'deman_agri_scbc'], pesos=[1, 1]):
    # B - COBACIA
    # AQ - deman_agri_scbc
    cobacia = df['COBACIA']
...
# =SOMASE($B:$B;$B2;AU:AU)
def ire_cs_irri_eco(df, parametros=['COBACIA', 'cs_ish_scbc_irri'], pesos=[1, 1]):
    cobacia = df['COBACIA']
...
# TODO: corrigir lógica, está entregando dados errado
# A ideia aqui é extrair uma coluna e colocar ao final para fazer os cálculos
def deman_pecuaria(df, parametros=[], pesos=[]):
    # demanda!D - Valor que eu quero
    # demanda!A - COBACIA
    # B - COBACIA
...
# =SE($H2<=10;$AF2*$AD2;0)
def deman_pecuaria_scbc(df, parametros=['situacao_setor', 'densidade_pec', 'area_setor'], pesos=[1, 1, 1]):
    # H2 - situacao_setor
    # AP2 - densidade
    # AN2 - area_setor
    situacao_setor = pd.to_numeric(df['situacao_setor'], errors='coerce')
...
def deman_pecuaria_otto(df, parametros=['deman_pecuaria_scbc', 'COBACIA'], pesos=[1, 1]):
...
def ire_cs_pec_eco(df, parametros=['ihu_cs_ish_pec', 'COBACIA'], pesos=[1, 1]):
...
# TODO: corrigir lógica, está entregando dados errado
# A ideia aqui é extrair uma coluna e colocar ao final para fazer os cálculos
def deman_indus(df, parametros=[], pesos=[]):
    # demanda!D - Valor que eu quero
    # demanda!A - COBACIA
    # B - COBACIA
...
def dem_ind_scbc(df, parametros=['situacao_setor', 'densidade_ind', 'area_setor'], pesos=[1, 1, 1]):
    # H2 - situacao_setor
    # AP2 - densidade
    # AN2 - area_setor
    situacao_setor = pd.to_numeric(df['situacao_setor'], errors='coerce')
...
def dem_ind_bacia(df, parametros=['dem_ind_scbc', 'COBACIA'], pesos=[1, 1]):
...
def ire_cs_ind_eco(df, parametros=['ihu_rel_ind', 'COBACIA'], pesos=[1, 1]):
