...
tabela_central = pd.read_csv('Indicador-Industria-Ind_sc_otto.csv', dtype=dtype_dict)
...
dados_entregues['densidade'] = (
    dados_entregues['deman_indus']/dados_entregues['area_otto']
).fillna(0).replace([float('inf'), -float('inf')], 0).astype(float)
...
# ADICIONANDO funções da tabela industrial
# AF
dados_entregues['pop_urb_scbc'] = np.where(
    dados_entregues['situacao_setor'] <= 10,
    dados_entregues['densidade']*dados_entregues['area_scbc'],
    0
)

# AG
dados_entregues['pop_urb_bacia'] = dados_entregues.groupby('COBACIA')['pop_urb_scbc'].transform('sum')

# AH
dados_entregues['perc_scbc'] = (
    dados_entregues['pop_urb_scbc']/dados_entregues['deman_indus']
).fillna(0).replace([float('inf'), -float('inf')], 0).astype(float)

# AL: igh_ind
dados_entregues['igh_ind'] = (
    dados_entregues['disp_q95']/dados_entregues['deman_indus']
).fillna(0).replace([float('inf'), -float('inf')], 0).astype(float)

# ihu_rel: multiplicação do percentual da população na porção do setor censitário e ihu_cs_ish (preciso de conceituação) 
